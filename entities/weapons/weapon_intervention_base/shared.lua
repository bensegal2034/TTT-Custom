--this base is the m9k weapon base, except I removed some stuff so that way it works with the other code. idk man
...
SWEP.DrawCrosshair			= true		-- crosshair = yes
...
function SWEP:Reload()
	...
	timer.Simple(waitdammit + .1,
		...
		self.Weapon:SetNWBool("Reloading", false)
		if self.Owner != nil and self.Owner.KeyDown and self.Owner:KeyDown(IN_ATTACK2) and self.Weapon:GetClass() == self.Gun then
		...
		elseif self.Owner.KeyDown and self.Owner:KeyDown(IN_SPEED) and self.Weapon:GetClass() == self.Gun then
